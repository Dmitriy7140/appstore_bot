...
- `DAILY_SALES_REPORT_CHAT_ID=-1004486126389` — группа для ежедневной сводки
  ...
  интервал API `[00:20; 00:20)`. Тем же запуском бот обновляет продажи и
  сумму в листе `app store bot` маркетинговой таблицы. Оставьте значение
  пустым, чтобы отключить обе ежедневные операции.
...
`/announce`. Команда `/codes` показывает свободный остаток App Store на складе
без действующих резервов и доступна только Telegram ID из `ADMIN_IDS`.
Коды загружаются через складскую админку; лимитов вместимости нет.
Команда `/refreshcodes` удалена. В логе должны появиться запуск SQLite и API webhook.
