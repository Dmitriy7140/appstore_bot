...
class AnalSheets:
    ...
    async def sync_links(self):
        try:
            ...
            # вся работа с gspread — в выделенном потоке, чтобы не морозить event loop
            await run_sheet(self._sync_links_blocking, db_rows)
            logger.info(f"Синхронизировали в таблице с ссылками: {len(db_rows)} рядов")

        except Exception as e:
            logger.error(f"Ошибка в синхронизации таблицы: {e}")

    def _sync_links_blocking(self, db_rows):
